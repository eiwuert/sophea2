...
?>
    ...
    <body>
		...
		<h4 class="header textCenter"><br>
			Product and Service Doctor Commissions
		</h4>
	    </div>
		...
		<tbody>
			...
			$n = 0;
                        $total = 0;
                        $dotor = '';
			foreach ($commission_report as $rows) {
                            
                            $prices = $rows->items_prices * $rows->items_qty;
                            $totalPer = $rows->assignper + $rows->acceptper;
                            $total += (($prices * $totalPer) / 100);
                            
                            if($rows->assignper > 0){
                                $doctor = $rows->assigner;
                            }
                            
                            if($rows->acceptper > 0){
                                $doctor = $rows->accepter;
                            }
				...
				echo '<td>'.$rows->items_modified.'</td>';
                                ...
                                echo '<td style="text-align: center;">'.$prices.'$</td>';
				echo '<td style="text-align: center;">'.$rows->assignper.'%</td>';
                                echo '<td style="text-align: center;">'.$rows->acceptper.'%</td>';
                                echo '<td style="text-align: right;">'.(($prices * $totalPer) / 100).'$</td>';
			    echo '</tr>';
			}
