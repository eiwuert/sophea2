<div class="content-wrapper" style="min-height: 916px;">
	...
	<section class="content-header">
		<h1>
			<?php echo '<a href="'.$base_url.'index.php/patient_rooms">'.$h_patient_room.'</a>';?>
				...
				<div class="box" style="padding: 10px !important;">
						...
						<br/>
							    ...
							    <div class="input-group">
								  	<div class="input-group-addon">
										...
										<?php echo form_dropdown('patient_id', @$drop_patient,'','class="form-control" id="patient_id"');?>
								  		<input type="text" name="ptr_id" id="ptr_id" style="display:none;">
							    </div>
							</div>
							<!--room Name-->
							<div class="form-group has-error">
							...
							<div class="form-group has-error">
							    <div class="input-group">
									<div class="input-group-addon">
									    <input type="checkbox" value="1" id="chart_storage_room">
									</div>
									<input type="text" value="Chart Storage" class="form-control" disabled="disable"/>  
								</div>
							</div>
							<div class="form-group has-error">
							      <div class="input-group">
										<div class="input-group-addon">
								  		...
								  		<input type="text" name="ptr_date" id="ptr_date" class="form-control">
							      </div>
							</div>
			...
			<div class="col-xs-12">
				<div class="box">
					...
					<table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
										<thead>
											<tr role="row">
												<th><?php echo $room.' '.$code;?></th>
												<th><?php echo $name.' '.$room;?></th>
...
<script>
    ...
    $(document).ready(function(){    	
	    $( "#ptr_date" ).datepicker({dateFormat: "dd-mm-yy",changeYear:true,changeMonth:true,yearRange: "1965:2030"});
    ...
    function getTypeList(mySearch,pageStart,pageLimit){
        ...
        var i = 0;
        var stRow = '';
        $.post("<?php echo $base_url;?>index.php/patient_rooms/get_ptr_list",{
            ...
            $.each(data, function(key,value) {
               		htmlView += '<tr ' + stRow + '>';
                    htmlView += '<td>' + value.room_code + '</td>';
                    htmlView += '<td>' + value.room_name + '</td>';
                    htmlView += '<td>' + value.patient_code + '</td>';
                    htmlView += '<td>' + value.patient_kh_name + '</td>';
                    htmlView += '<td>' + value.ptr_date + '</td>';
                    htmlView += '<td>';
                    	
                        htmlView +='<a href="#" title="<?php echo @$edit;?>"><i class="fa fa-edit action-btn primary" onclick="editPtr(' + value.ptr_id + ');"></i></a>';
                        htmlView +='<a href="#" title="<?php echo @$delete;?>"><i class="fa fa-trash-o action-btn danger" onclick="deleteWaitting(' + value.ptr_id + ');"></i></a>';
                    htmlView += '</td>';
                htmlView += '</tr>';
            });
    ...
    function editPtr(ids){
	$('#msgs').css('display','none');
	$('#form_row').css('display','block');
	$('#form_table').css('display','none');
	$('#submit_edit').val('Update');
	$('#title_name').html('/ Edit');
	$.post("<?php echo $base_url;?>index.php/patient_rooms/get_ptr_info_by_id_json/"+ids,
		function(data,status){
		       $.each(data, function(key,value) {
			       $('#ptr_id').val(value.ptr_id);
			       $('#patient_id').val(value.ptr_patient_id);
			       $('#room_id').val(value.ptr_room_id);
			       $('#ptr_date').val($.datepicker.formatDate('dd-mm-yy', new Date(value.ptr_date)));
			       if(value.ptr_chart_storage_room > 0){
			       		$('#chart_storage_room').prop('checked', true);
			       }
    ...
    function saveEdit(){
		...
		var ptr_patient_id = $('#patient_id').val();
		var ptr_room_id = $('#room_id').val();		
		var ptr_chart_storage_room = $('#chart_storage_room').val();		
		var ptr_date = $('#ptr_date').val();		
		$.post("<?php echo $base_url;?>index.php/patient_rooms/save_ptr",{
		    ptr_id: ptr_id,
		    ptr_patient_id: ptr_patient_id,
		    ptr_room_id: ptr_room_id,
		    ptr_chart_storage_room: ptr_chart_storage_room,
		    ptr_date: ptr_date
		},function(data,status){
    ...
    function deleteWaitting(ids){
	    $.post("<?php echo $base_url;?>index.php/waittings/delete_waitting/"+ids,{ptr_id: ids},function(data,status){}); 
	    pagination();            
    }
