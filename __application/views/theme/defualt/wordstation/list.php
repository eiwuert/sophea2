<div class="content-wrapper" style="min-height: 916px;">
	...
	<section class="content-header">
		<h1>
			<?php echo '<a href="'.$base_url.'index.php/workStations">'.$h_workstation.'</a>';?>
						...
						<br/>
						  ...
						  <div class="form-group">
							<div class="input-group">
							  ...
							  <input type="text" name="workstation_id" id="workstation_id" style="display:none;">
							</div>
						  </div>
						  ...
						  <div class="form-group">
							<div class="input-group">
							  ...
							  <textarea name="workstation_desc" id="workstation_desc" class="form-control" rows="7"></textarea>
							</div>
						  </div>
			...
			<div class="col-xs-12">
				<div class="box">
					...
					<table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
										<thead>
											<tr role="row">
												<th><?php echo $workstation;?></th>
												<th><?php echo $desc;?></th>
												<th></th>
											</tr>
										</thead>
										<tbody id="workstationList"></tbody>
									</table>
...
<script>
    ...
    function getWorkstationList(mySearch,pageStart,pageLimit){
        ...
        var i = 0;
        var stRow = '';
        $.post("<?php echo $base_url;?>index.php/workStations/get_workstation_list",{
            ...
            $.each(data, function(key,value) {
                htmlView += '<tr ' + stRow + '>';
                    htmlView += '<td>' + value.workstation_name + '</td>';
                    htmlView += '<td>' + value.workstation_desc + '</td>';
                    htmlView += '<td>';
                        htmlView +='<a href="#" title="<?php echo @$edit;?>"><i class="fa fa-edit action-btn primary" onclick="editWorkstation(' + value.workstation_id + ');"></i></a>';
                        htmlView +='<a href="#" title="<?php echo @$delete;?>"><i class="fa fa-trash-o action-btn danger" onclick="deleteWorkstation(' + value.workstation_id + ');"></i></a>';
                    htmlView += '</td>';
                htmlView += '</tr>';
            });            
            $("#workstationList").html(htmlView);
	...
	// get edit
	$.post("<?php echo $base_url;?>index.php/workStations/get_workstation_info_by_id_json/"+ids,
	function(data,status){
	       $.each(data, function(key,value) {
		       $('#workstation_name').val(value.workstation_name);
		       $('#workstation_desc').val(value.workstation_desc);
		       $('#workstation_id').val(value.workstation_id);
	       });
	});
    }
    
    function saveEdit(){
		var workstationId = $('#workstation_id').val();
		var workstationName = $('#workstation_name').val();
		var workstationDesc = $('#workstation_desc').val();		
		$.post("<?php echo $base_url;?>index.php/workStations/save_workstation",{
		    workstation_id: workstationId,
		    workstation_name: workstationName,
		    workstation_desc: workstationDesc
		},function(data,status){
    ...
    function deleteWorkstation(ids){
	    $.post("<?php echo $base_url;?>index.php/workStations/delete_workstation/"+ids,{workstation_id: ids},function(data,status){}); 
	    pagination();            
    }
    ...
    function pagination(ids){
		...
		getWorkstationList(mySearch,pageStart,pageLimit);
	}
