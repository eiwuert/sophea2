<div class="content-wrapper" style="min-height: 916px;">
	...
	<section class="content-header">
		<h1>
			...
			<small id="title_name">/ <?php echo $room;?> </small>
		</h1>
	</section>
						...
						<br/>
						  ...
						  <div class="form-group">
							<div class="input-group">
							  ...
							  <textarea name="room_desc" id="room_desc" class="form-control" rows="7"></textarea>
							</div>
						  </div>
...
<script>
    ...
    function getTypeList(mySearch,pageStart,pageLimit){
        ...
        var i = 0;
        var stRow = '';
        $.post("<?php echo $base_url;?>index.php/rooms/get_room_list",{
            ...
            $.each(data, function(key,value) {
                htmlView += '<tr ' + stRow + '>';
                    ...
                    htmlView += '<td>' + value.room_desc + '</td>';
                    htmlView += '<td>';
                        htmlView +='<a href="#" title="<?php echo @$edit;?>"><i class="fa fa-edit action-btn primary" onclick="editRoom(' + value.room_id + ');"></i></a>';
                        htmlView +='<a href="#" title="<?php echo @$delete;?>"><i class="fa fa-trash-o action-btn danger" onclick="deleteRoom(' + value.room_id + ');"></i></a>';
                    htmlView += '</td>';
                htmlView += '</tr>';
            });            
    ...
    function editRoom(ids){
	$('#msgs').css('display','none');
	$('#form_row').css('display','block');
	$('#form_table').css('display','none');
	$('#submit_edit').val('Update');
	$('#title_name').html('/ Edit');
	$.post("<?php echo $base_url;?>index.php/rooms/get_room_info_by_id_json/"+ids,
	function(data,status){
	       $.each(data, function(key,value) {
		       $('#room_code').val(value.room_code);
		       $('#room_name').val(value.room_name);
		       $('#room_desc').val(value.room_desc);
    ...
    function saveEdit(){
		...
		$.post("<?php echo $base_url;?>index.php/rooms/save_room",{
		    room_id: roomId,
		    room_code: roomCode,
		    room_name: roomName,
		    room_desc: roomDesc
    ...
    function deleteRoom(ids){
	    $.post("<?php echo $base_url;?>index.php/rooms/delete_room/"+ids,{room_id: ids},function(data,status){}); 
	    pagination();            
    }
