...
class Visitor extends Datastructure{
	...
	function getPatientVisitReport(){
        ...
        function getDrugReport(){
        	$select = " pro.products_name, SUM(si.items_qty) as sumQty, SUM(si.items_prices) as sumPrice";
            $from = $this->getTblServiceItem()." AS si";    
            $from.= " JOIN emedirec_1_products as pro ON pro.products_id = si.products_id";         
            $where = " si.items_deleted = 0 GROUP BY si.products_id";
            
            if($this->getDate1() != '' || $this->getDate2() != ''){
                $where .= " AND (accepted_date BETWEEN '".date('Y-m-d',strtotime($this->getDate1()))."' AND '".date('Y-m-d',strtotime($this->getDate2()))."')";
            }
